  ...
  if (available.length === 0) {
    ...
    return (
      <button
        ...
        title={t('handoff.noEditors', { target: fallbackLabel })}
    ...
    >
      ...
      <div className="handoff-split">
        <button
          ...
          title={primary ? t('handoff.openIn', { editor: primary.label }) : t('handoff.open')}
          onClick={() => {
          ...
          {primary ? (
            <>
              ...
              <span className="handoff-trigger-label">
                {t('handoff.openIn', { editor: primary.label })}
              </span>
            </>
          ) : (
            <>
              <EditorIcon editorId="finder" size={20} />
              <span className="handoff-trigger-label">{t('handoff.open')}</span>
            </>
          )}
        </button>
        <button
            ...
            <>
              ...
              {unavailable.map((editor) => (
                <button
                  ...
                  onClick={() => void launch(editor)}
                  disabled={busy === editor.id}
                  title={t('handoff.notDetected', { editor: editor.label })}
                >
                  <EditorIcon editorId={editor.id} size={20} />
                  <span>{editor.label}</span>
                </button>
